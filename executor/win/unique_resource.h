...
public:
	...
	// construction
	explicit unique_resource(R &&resource, D &&deleter,	bool shouldrun = true) noexcept(true)
	...
	// resource access
	R const &get() const noexcept(true) 
	{ 
		return this->resource; 
	}

	// resource access
	R& get_unsafe() noexcept(true)
	{
		return this->resource;
	}

	operator R const &() const noexcept(true)
	{
		return this->resource; 
	}

	R operator->() const noexcept(true) 
	{
		return this->resource; 
	}
	...
	// deleter access
	const D &get_deleter() const noexcept(true) 
	{ 
		return this->deleter; 
	}
};
